...
class Settings(object):
    ...
    qz_limit = 1.31
    x_side_mm = 2. * distance_to_detector_mm * np.tan( 2. * np.arcsin(qy_limit/4./np.pi)) * wavelength_nm
    y_side_mm = distance_to_detector_mm * np.tan( 2. * np.arcsin(qz_limit/4./np.pi)) * wavelength_nm
    u0_mm = x_side_mm/2.0
    v0_mm = - distance_to_detector_mm * np.tan(alpha_i*np.pi/180.0)
    ###
...
def new_instrument(spherical_detector=False):
    ...
    instrument.getOptions().setMonteCarloIntegration(False, 10)
    return instrument
