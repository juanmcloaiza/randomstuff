...
def get_sample(height=50, orientation='parallel_to_beam'):
    # Defining Materials
...
def get_simulation(alpha_i=0.15, side_bins=500, nMonteCarlo=100):
    ...
    simulation.getOptions().setMonteCarloIntegration(True, nMonteCarlo)
...
def run_simulation(alpha_i, height, orientation, side_bins, nMonteCarlo):
    sample = get_sample(height=height, orientation=orientation)
    simulation = get_simulation(alpha_i=alpha_i, side_bins=side_bins, nMonteCarlo=nMonteCarlo)
    simulation.setSample(sample)
    simulation.runSimulation()
    return simulation.result()


def mySaveResult(baResult=None):
    Imatrix = baResult.array()
    qvect_axes_limits = ba.get_axes_limits(baResult, ba.AxesUnits.QSPACE)
    angle_axes_limits = ba.get_axes_limits(baResult, ba.AxesUnits.DEGREES)
    nbins_axes_limits = ba.get_axes_limits(baResult, ba.AxesUnits.NBINS)
    np.save("./Imatrix.npy", Imatrix)
    np.save("./qvect_axes_limits.npy", qvect_axes_limits)
    np.save("./angle_axes_limits.npy", angle_axes_limits)
    np.save("./nbins_axes_limits.npy", nbins_axes_limits)
...
if __name__ == '__main__':
    for alpha_i in [0.15, 0.40]:
        ...
        for height in [50.0, 100]:
            ...
            for nMC in [10,20,50,100]:
                print("\t nMC = {}...".format(nMC))
                for nBins in [50,100,250,500]:
                    print("-"*60)
                    print("\t nBins = {}...".format(nBins))
                    ta = time.time()
                    result = run_simulation(alpha_i=alpha_i,
                                            height=height,
                                            orientation='parallel_to_beam',
                                            side_bins=nBins,
                                            nMonteCarlo=nMC)
                    tb = time.time()
                    print("\t time [s] = {}...".format(tb-ta))
                    mySaveResult(result)